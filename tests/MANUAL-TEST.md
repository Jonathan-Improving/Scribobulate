...
- [ ] **25.23** Export `tests/fixtures/large-doc.md` to PDF → after about half a second the footer shows "Exporting page P of N…" with a progress bar and **Cancel**, P climbing while the window repaints; File ▸ Export is greyed out meanwhile. Press Cancel → it stops after the current page, the indicator disappears, and no file is written (a pre-existing destination stays byte-identical, 25.21). Export again and close the window mid-export → the export stops first, then the window closes. A one-page document exports without the indicator ever appearing (TDD 25.23)
