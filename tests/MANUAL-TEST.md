...
- [ ] **15.8** Two tabs, one Preview one Split (different orientation) → switching between them restores each tab's own stored view mode/orientation; the swapped-pane order stays whatever the window is already showing on both tabs
- [ ] **15.8a** Split mode, Swap Panes → arrangement flips; File ▸ New Document → the new tab opens ALREADY swapped, not reset to the default order; switch back to the first tab → still swapped too. Then View ▸ Move Tab to New Window (or drag a tab into a second, un-swapped window) → the moved tab adopts the DESTINATION's (un-swapped) order, and the two windows stay free to disagree (TDD 15.8a)
- [ ] **15.9** 2+ tabs, change zoom → every tab's preview in that window rescales, other windows' zoom unaffected
- [ ] **15.10** 2+ windows, 2+ tabs each, mixed modes/orientations/zooms, one window's panes swapped, quit, relaunch → every window's size/zoom/pane-order and every tab's mode/orientation/active-tab restored exactly
