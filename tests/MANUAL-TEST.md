...
- [ ] **25.9a** **Defect, found in QA round 3 (2026-09-19): the PDF sink consumed pixel dimensions as points.** Set a theme's `table_head_scene` to a picture of a known pixel width (say 96px) with `table_head_scene_anchor` set to a corner, plus a `rule` sprite and a `blockquote_bg` tile. Export a document with a table, a horizontal rule and a multi-line quote, then **measure the artefact**: `pdfimages -list out.pdf`, or measure the corner scene against the page in a PDF editor at 100% → 96px must print as **72pt**, not 96pt. ⚠️ **Measure one decoration against the PAGE, never two decorations against each other**: the error was a uniform 4/3 across every themed image, so everything looked consistent and deliberate, and the only way to see it was to hold the page beside the preview at 100% zoom — which §25.9 already asks for the two *metric* keys and did not ask for images. Check the tiled quote panel too: its lattice must not restart at each text row, and a bar tile sized to `blockquote_bar_width` must not be clipped down its right edge (TDD 25.9, 18.60)
