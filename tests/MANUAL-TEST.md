...
- [ ] **25.23** Export `tests/fixtures/large-doc.md` to PDF → after about half a second the footer shows a progress bar and **Cancel**, and a page count that climbs while the window repaints. ⚠️ **The message reads plain "Exporting…" until the page count is known** — pagination runs before the first page is drawn, and only then can "Exporting page P of N…" name anything. How long that lasts is platform-specific and none of it is a defect: MEASURED blocked for ~1.6s on Windows (no repaint, menus will not even map), NOT blocked on macOS (window repaints and Cancel is offered throughout — a 5628-page fixture sat on "Exporting…" for 22s+), and on Linux a document large enough (31MB) can stay there for minutes because the preview's own layout work outranks export's page drawing. Use the 3MB fixture, where the count appears within a couple of seconds; meanwhile the Export submenu's **PDF and HTML rows** are greyed. ⚠️ Read the CHILD rows, not the `Export` row itself: that row is a submenu header carrying no action, so it stays black however the command is gated and a check that reads it alone passes vacuously (MEASURED, Windows seat). Press Cancel → it stops after the current page, the indicator disappears, and no file is written (a pre-existing destination stays byte-identical, 25.21). Export again and close the window mid-export → the export stops first, then the window closes. A one-page document exports without the indicator ever appearing (TDD 25.23)
