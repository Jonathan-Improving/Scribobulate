...
- [ ] **11.13a** **A tab keeps its own options, and a restart keeps them but not the search** (TDD 15.12). Open two tabs. In tab 1 tick **Aa** and search a term; in tab 2 tick **Reg-Ex** and search another. Switch between them → each tab's bar shows **its own** query AND its own toggles, and the Edit-menu ticks follow. Quit and reopen → each restored tab still has its own toggles (open the bar to see them), **but the find bar is closed and no search is in force**. ⚠️ The half that fails quietly is the toggles reverting on a tab switch while the search still behaves the old way, or the reverse — behaviour and mirror must move together.
