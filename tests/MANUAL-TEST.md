...
- [ ] **11.5a** **Escape closes the bar from anywhere in the window** (TDD 11.11). Ctrl+F, type a term, press **Enter** — the caret is now back in the document, which is the normal way to be sitting with the bar open. **Without clicking back into the bar**, press **Escape** → the bar closes. ⚠️ This is the leg that was broken while 11.5 passed, because 11.5 is naturally run with the focus still in the search field. Then the half that matters more, because getting it wrong is worse than the bug: with the bar **open**, open a **menu** (or any popover — the tab-list button, a toolbar drop-down) and press Escape → the **popover** closes and the **find bar stays open**. Repeat with the **annotation card** (annotate a selection) and with **Go To Line**: each takes its own Escape first, and only a second Escape reaches the bar. Finally close the bar and press Escape with nothing open → nothing happens, no console noise
