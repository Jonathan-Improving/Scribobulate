...
- [`sdd/PLAN.memory-gates.md`](sdd/PLAN.memory-gates.md) — **kept after implementation** (operator decision): the measured per-render WebP leak, the rejected approaches, and the gate's construction. The gate itself is pipeline step 5b (TDD 6.6–6.8); the local-image cache and the non-leaking dimension probe are in `renderer/start.rs` / `imagecache/`. **Read it before changing the image decode path, before altering the memory-gates step, and before assuming a re-render is free.** Method is not restated here — it consumes PLAN.profiling.md's T3 ladder.
