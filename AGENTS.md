...
- [`sdd/PLAN.memory-gates.md`](sdd/PLAN.memory-gates.md) — deferred: a diagnosed per-render memory-retention defect (a local image is decoded on every render and the decode is retained; an animated WebP amplifies it to ~25 MB per re-render, measured to 2.7 GB over 98 theme switches) and the leak gate that would have caught it — a third test class beside unit and integration, deterministic, outside the coverage ratchet, with its own mandatory pipeline step bounding **per-render growth**, not absolute RSS. **Read it before changing the image decode path, before adding a memory gate, and before assuming a re-render is free.** It records which fixtures do and do not reproduce the leak, and four measurement traps that each produced a clean-looking false negative. Method is not restated here — it consumes PLAN.profiling.md's T3 ladder.
