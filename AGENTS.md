...
- [`sdd/PLAN.memory-gates.md`](sdd/PLAN.memory-gates.md) — **RETIRED, both phases shipped**, kept for its measured evidence: the per-decode leak numbers, the route-by-route table showing which gdk-pixbuf entry point leaks how much, and the C-program corroboration. **Read it before touching an image decode path, the animation machinery, or the memory-gates step (5b)** — the live rules are in TECH.md, POLICY.md and ScrAP-351/ScrAP-352, but the numbers that justify them are only here. Method is not restated: it consumes PLAN.profiling.md's T3 ladder.
