...
- **Image decoding and animation are implemented, not planned** — the memory-gates plan retired 2026-09-16 once both phases were delivered and ratified. Before touching an image decode path, the animation machinery, or the per-render memory-growth step (5b), read **ScrAP-351 and ScrAP-352** (why a third-party gdk-pixbuf loader module leaks on every decode, why only a growth *slope* can see it, and why two entry points into one module behave as two decoders) together with [`sdd/TECH.md`](sdd/TECH.md)'s `imagedecode/` and `animation/` entries, [POLICY's per-render memory-growth class](sdd/POLICY.md#per-render-memory-growth-class), and [`sdd/TDD.md`](sdd/TDD.md) §6.6–6.10 and §27. `src/imagedecode` is the one decode choke point and `clippy.toml` bans every GTK entry point that routes around it.
- [`sdd/PLAN.narrow-window.md`](sdd/PLAN.narrow-window.md) — in progress: the toolbar now wraps per button (with a few closely-related clusters kept atomic, e.g. Back/Forward, Zoom in/reset/out) instead of hard-locking below its content-derived minimum width, and `MIN_WINDOW_WIDTH` no longer masks that wrap. **Read it before touching `MIN_WINDOW_WIDTH`, the toolbar's section show/hide reconciliation (`viewactions.rs` I3–I7), `window::toolbar`'s per-section item lists, or `chrome_fit.rs`'s overflow-inset** — it records why active-shrink is deliberately off today, why `GtkFlowBox` was tried and reverted for `ToolbarWrapBox`, and what's still open (no verification against a real narrow display; the tab strip and sidebar are unrelated non-wrapping contributors, still out of scope).
