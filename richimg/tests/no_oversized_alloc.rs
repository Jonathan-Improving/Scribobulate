...
//! source — hence its own test binary target: the allocator instruments
//! every allocation this whole process makes, so sharing it with other
//! richimg tests would make the "no huge allocation happened" assertion
//! depend on what else happened to run in the same binary.
...
/// allocations (Vec growth, the decoder's own bookkeeping) that have nothing
/// to do with a pixel canvas.
...
fn read_oversized_canvas_fixture() -> Arc<[u8]> {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/oversized_canvas.webp");
    let bytes =
...
fn oversized_canvas_refuses_without_allocating_a_canvas_sized_buffer() {
    ...
    // probe() itself does not enforce the cap (see richimg::probe's doc
    // comment) — it should still see the (bogus, oversized) dimensions
    // without allocating anything remotely canvas-sized.
...
fn an_ordinary_small_fixture_still_decodes_under_the_counting_allocator() {
    ...
    let path =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/lossy_still.webp");
    let bytes = std::fs::read(&path).expect("reading lossy_still.webp");
    let limits = Limits::default();
