//! TDD 2.23a-b: an over-cap IHDR (16384x16384 in a tiny file) must refuse with
//! `Error::TooLarge` from `Animation::new` and `first_frame` WITHOUT ever allocating a
//! canvas-sized buffer, and `probe` must read the bogus dimensions without allocating a
//! pixel row that big either.
//!
