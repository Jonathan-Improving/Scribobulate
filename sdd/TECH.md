...
| `imagecache/` | Owns whether a decoded image needs fetching or decoding at all: a process-wide store of textures, keyed by URL for remote images and by path+mtime+size for local ones, in front of `imagefetch` and the local decode path, with the eviction and negative-caching policy as a display-free core. It exists because a render is no longer a rare event — a disclosure fold toggle or a theme switch re-walks the whole document — and because the gdk-pixbuf animated-WebP loader leaks per decode, so a re-render must not invoke it again. |
