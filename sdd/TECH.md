...
| `platform/x11.rs` | X11 builds only (`#[cfg]`-gated at its declaration in `platform/mod.rs`; a runtime display-type check makes it answer nothing on any other backend). Supplies where a point on a surface lies on the screen — the root-coordinate translation GTK's X11 backend performs to pick a popover's monitor and does not export — so `saferizer::popover_anchor` can keep every anchor on a monitor. Owns no decision. |
...
| `platform/mac/fullscreen.rs` | macOS only (`#[cfg]`-gated at its declaration in `platform/mod.rs`). Supplies the window *classification* GDK's Quartz backend never makes: every toplevel is tagged a candidate primary fullscreen window at construction, with no dialog/transient exemption, so attaching a transient child to a parent already in a fullscreen Space makes AppKit run a real fullscreen transition on the child instead of floating it (GTK4Rs/AP-277). Subscribes once to GTK's toplevel list from `app::setup::on_startup`; for each secondary window (transient parent or modal) it withholds `transient-for` until realize, reclassifies the now-existing `NSWindow` as *auxiliary* via hand-rolled Cocoa FFI (the Objective-C runtime plus the one public GDK entry point `gdk_macos_surface_get_native_window`, 4.8+), then restores the parent — so GTK attaches the child exactly as it would have, against a window AppKit will not try to make fullscreen. Document windows are untouched. |
| `platform/mac/bundle.rs` | macOS only (`#[cfg]`-gated at its declaration in `platform/mod.rs`). Owns deciding whether this process is running from inside a `.app` and, when it is, pointing GTK's runtime-data lookups at the bundle's own staged copies — icon theme, GSettings schemas, GtkSourceView validators and the gdk-pixbuf loader cache. It stages nothing: `packaging/macos/bundle.sh` puts those files in the bundle, and this module is what makes a launched process find them rather than a prefix that may not exist on the user's machine. |
| `platform/mac/pointercrossing.rs` | macOS only (`#[cfg]`-gated at its declaration in `platform/mod.rs`). Supplies the pointer-crossing events GDK's Quartz backend never delivers to a toplevel, without which GDK discards every cursor the application sets (the module's own header carries the source trace and the upstream issue). Subscribes once to GTK's toplevel list from `app::setup::on_startup` — the defect is per surface, so each new window needs arming — and gives each window's surface an input region at realize. It sets no cursors and owns no hover behaviour: it restores a precondition the rest of the application's cursor handling already assumes on every other platform. |
| `platform/mac/pasteboard.rs` | macOS only (`#[cfg]`-gated at its declaration in `platform/mod.rs`). Writes text to the general pasteboard **eagerly, as data**, by raw `objc_msgSend`. GTK 4.22's Quartz backend only ever publishes a lazy promise, and answering one can livelock the process when the request lands inside GDK's own event-source `prepare()`; any eager clipboard reader (clipboard-history or clipboard-sharing apps) makes that likely. Fixed upstream in GTK 4.24.0. Called only from `clipboard::set_text`, straight after GDK's own write. |
...
| `window/toolbar.rs` | Owns toolbar construction. A visually delimited section is a **list** of individually-wrappable items — its leading separator, then single buttons or small clusters — not one container, which is what lets the toolbar wrap per button; `win.show-tbtn-<id>` still hides a section as one unit by toggling every item in its list (invariant I2). **Every** section is decomposed, Format included: it was the last opaque one, and at ~555px it alone set the window's minimum width. Also owns the rule that no toolbar item's width may be unbounded — the two value-bearing combos are character-capped, because the wrap box's minimum is the widest item it holds. |
| `window/splitview.rs` | Owns `SplitView`, the per-tab orderable two-pane splitter widget. |
| `window/arrangement.rs` | Owns the app-wide split arrangement (pane order and orientation): the `app.split-swap` / `app.split-orientation` actions that hold it and re-apply it to every tab of every window. Each window's `win.` actions only forward to them, and exist per window **because sensitivity is per window** — the one reason a forwarder layer is ever warranted, and the reason `window/toolbarchrome.rs` has none. |
...
| `window/toolbarchrome.rs` | Owns the app-wide toolbar layout: the `app.show-toolbar` and six `app.show-tbtn-<id>` actions, the invariants I1–I8 that govern them, and the re-apply that reaches every open window's bar. **One layer, no `win.` forwarders** — unlike the split arrangement beside it nothing here is per-window, sensitivity included. |
| `window/viewactions.rs` | Owns registration of the view, sidebar, status-bar, split and zoom `win.*` actions — the chrome that is genuinely per window. |
| `window/editbar/formatbar.rs` | Owns the Format commands as an ordered list (`format_items`) and, separately, as a single packed row (`build_format_bar`). Two shapes of one thing, because the consumers differ: the toolbar packs the items individually so a narrow window can wrap them, while the Stage-2 caret overlay is a popover and wants one row. Building both from the same list is what stops the two surfaces drifting. |
...
| `window/find.rs` | Owns the find engine for both the editor and preview panes and resolves which of them a find acts on, plus the two find values the per-tab state holds: the match cursor and the preview hit-list cache. The preview's half is assembled here from `find/`'s pure parts; the editor's is `GtkSourceSearchContext`, which implements the same options natively. |
...
| `window/reload.rs` | Owns live-reload and conflict handling for external file changes, including the settle re-read that tells a document that really lost its backing from one caught mid-replacement — blank and absent are both states a file passes *through* while being rewritten, so both cost the same wait. |
| `window/backingloss.rs` | Owns applying a document's *backing loss* — its file deleted or emptied on disk, leaving the buffer as the only copy — to every guard that reads it: Save, the ⚠ badge, the close prompt, the crash-recovery snapshot, the persistent status line and the floating Save/Dismiss prompt. The last two are one fact on two surfaces by design: the line states the condition for as long as it holds, the prompt carries the control, and dismissing the prompt lowers no guard. |
...
| `window/wheelcoalesce.rs` | Owns the wheel input of a scroller that holds a virtualized list: it takes the scroll event in the capture phase, accumulates the travel, and applies it once per frame. A pre-4.10.1 `GtkListView` resolves the second adjustment write of a frame against a row-height tree its own first write left collapsed, and snaps to the end of the list (GTK4Rs/AP-114); one write per frame removes that precondition. Inert on a GTK that fixed it. |
| `window/zoomwheel.rs` | Owns the modifier+wheel zoom input on the preview scroller: which modifier the host spells as the command key, how much wheel travel is a rung of the ladder, and which `win.zoom-*` action that rung activates — an extra input to those actions, owning no zoom behaviour of its own. Installed by `preview::render`, the one site a preview scroller is built, so a preview cannot ship without it. |
...
| `widgets/tab/` | Owns the tab-strip widget: `TabBar`, its rows, and their drag, close and context-menu mechanics. Positions are *cached* (a resting slot plus an animated position) and derived from the running total of the handle widths to a tab's left, so it also owns the one funnel every width-changing mutation goes through to re-derive them — GTK's `queue_resize` re-runs the layout against the stale cache (GTK4Rs/AP-290). |
| `widgets/disclosure.rs` | Owns the disclosure toggle: the anchored `GtkToggleButton` a summary line carries, its indicator, the CSS class by which a press that landed on the control is told apart from one that landed on the line around it, and **the place in the document the control names** — a `docref::AnchoredSpan` held on the widget itself, so a control that survives a fold splice keeps its own reference rather than being re-paired out of a list by position. |
...
| `widgets/sprite_icon.rs` | Owns `SpriteIcon`, a theme sprite drawn at a fixed size — the disclosure indicator's sprite shape, and an animated heading marker's (a still marker stays a buffer paintable) — drawn rather than pictured so an animated sprite plays. |
| `widgets/wrapbox.rs` | Owns `ToolbarWrapBox`, the left-packed wrapping row the toolbar is built on: children keep their natural size and overflow onto a new row rather than being clipped or squeezed, so the window's content-derived minimum width is the widest **single** item instead of the sum of every visible one. Measures and allocates itself (`HeightForWidth`, no layout manager) through one shared packing function, so the height it promises and the rows it lays out cannot drift apart. `GtkFlowBox` cannot do this job — it fits a shared column grid across every row, which offsets a later row whose items differ in size from the row that set the grid — not reachable by any property, `homogeneous` and `min_children_per_line` included. |
...
| `codeview/card.rs` | Owns `AnnotationCard`, the `GtkPopover` subclass an annotation chip opens: its anchor (the annotation's identity, never a rectangle), its scroll-tracking, its dismissal and its teardown — and, because it is its own `GtkNative` and therefore outside the window's accelerator reach, the local re-offer of every application accelerator (GTK4Rs/AP-266). |
| `codeview/navkeys.rs` | Owns keeping the preview's navigation keys reachable when an anchored table cell holds the focus: a capture-phase key controller — the only phase that still sees a key a focused selectable `GtkLabel` has claimed — re-emitting the `move-cursor` GTK's own binding would have. Decision-free; it reads `keynav` (GTK4Rs/AP-264). |
...
| `renderer/disclosure/identity.rs` | Owns what a disclosure block is CALLED — the source range of its opening delimiter (`<details …>` through the `</summary>` that labels it), which is what a control anchors on so it can find its block again after an edit. Widest span that survives the reader typing INSIDE the block, narrowest that carries the label; the synthetic front-matter block has no such tag, so it anchors on the opening fence line and relies on front matter being at byte 0. |
| `renderer/frontmatter.rs` | Owns where a document's front matter is (YAML `---`, TOML `+++`) and, as the **walk seam every whole-document parse reads a document through**, what each sink sees there: a collapsed `<details>` around a fenced code block, or nothing at all (TDD 2.27). It emits synthetic events tiling the block's real source bytes rather than rewriting the document, so the renderer needs no front-matter branch and every offset map stays keyed to what the author typed. |
...
| `tags.rs` | Owns the preview's `GtkTextTag` set and its style setup — the GTK half, including the inline-code chip's **one tag per surface** (`CODE_INLINE_SURFACES`), which exists because a tag carries a single background and cannot ask what the paint plan drew behind the run, and the code block's body tag **per quote depth**, registered after the `bq-{depth}` family so it wins the one `left-margin` both set. Its child `spec` owns the theme→tag DECISIONS as display-free values (the quote-ink floor's condition, the band inset's per-level gate, the design-time→pixel arithmetic, and the two block INDENTS — `list_indent_px` and `quote_indent_px` — which `renderer/` also reads, so the tag that applies a margin and the anchored-child bound that must clear it cannot round it differently), so they are assertable without a live buffer. |
| `span.rs` | Owns `BufferSpan`, the named character-offset range the renderer uses to record where each block landed, and the two spans that pair one with a nesting depth so a painter never re-derives it: `QuoteSpan` (a bar per level) and `CodeBlockSpan` (a card inset within the quote it sits in, rather than drawn over the panel). |
...
| `keynav.rs` | Owns which document movement a navigation key means — a mirror of `gtktextview.c`'s own binding table — and whether the widget holding focus makes that movement the document's to perform. Display-free, so both are settled by unit test (GTK4Rs/AP-264). |
| `farscroll.rs` | Owns every far scroll in either pane, and the one fact they all need: *when GTK has finished laying the document out*. Until it has, a scroll aimed past the validated frontier is uncomputable or silently dropped, so both the buffer-ends key wiring and the mark-based navigation seam re-issue against a settled layout (GTK4Rs/AP-260). |
...
| `outline/` | Owns the display-free document-outline model, and `HeadingSite` — where each heading the *source* declares is reachable in a given render, which is not the same list as the headings that reached the buffer. `tree` holds the flat-list → forest fold and the scroll-spy's ancestor-chain lookup, split out of `mod.rs` once `extract_headings` grew its own exhaustive `Event` match (POLICY's file-size soft limit). |
| `docref.rs` | Owns **`AnchoredSpan`**, the one way this application holds a place in a document across time: a byte range plus the text that occupied it, re-resolved against the live source at USE. The per-construct decision it carries is the ambiguity policy — nearest-occurrence for a distinctive identity (an annotation's claim), refuse-and-re-derive for one a document repeats (a disclosure's opening delimiter). Its contract in one line: **an unresolvable reference obliges the caller to re-derive the view, never to do nothing** — the difference between a wasted rebuild and a live control that silently does nothing, which is the defect the type's second home was written after (ScrAP-187, [CAM.md](CAM.md) § Document-Reference). Pure and display-free. |
| `fold.rs` | Owns the display-free fold model: which disclosure blocks are collapsed, keyed on cleaned-source offset. Read by the renderer as an input; holds no GTK type. The map is dropped whenever the text those keys index moves — which is a question with a MODE in it (`TabState::previewed_source`), not a property of any one field. |
| `outline_view.rs` | Owns the outline sidebar widget. A row carries the heading's durable title PATH as well as its build-local index, and an activation hands over the path — `window/outline_nav.rs` re-derives both the index and the source offset from the live document, so a click inside the live-edit debounce lands on the heading rather than on the place it used to be. |
...
| `annotate/` | Owns the CriticMarkup annotation core: scanning annotations out of the source, and the pure, total mutations that add, edit and remove them. |
...
| `imagecache/` | Owns whether a decoded image needs fetching or decoding at all: a process-wide store of textures, keyed by URL for remote images and by path + **modification time + LENGTH** + drawn size for local ones, in front of `imagefetch` and the local decode path, with the eviction and negative-caching policy as a display-free core. Length is in the key because mtime alone is not a change signal on any platform — Windows' `CopyFileExW` (which is `std::fs::copy`, Explorer's copy/paste and robocopy's default) carries the SOURCE's timestamp onto the destination, and `cp -p`, `rsync --times` and `unzip` do the same everywhere, so a replaced image kept serving the stale decode. Its `loader` submodule owns the load paths themselves — local raster, the SVG re-render at a target size, and the remote fetch — and the animation hint that lets a cache HIT rebuild a playing animation without re-reading. It exists because a render is no longer a rare event (a fold toggle or a theme switch re-walks the whole document) and because the decode it fronts used to leak per call (ScrAP-351). |
| `imagedecode/` | Owns the **only** route from encoded image bytes to pixels. It sniffs the format from the CONTENT — never the extension — and routes WebP, GIF and APNG to `richimg`, building a `MemoryTexture` from the decoded frame; everything else (a still PNG, JPEG, SVG) goes to GTK's own decoder. A format `richimg` claims is never retried through GTK, so the leaking loader is unreachable rather than merely avoided (ScrAP-351, ScrAP-352), and `clippy.toml` bans GTK's encoded-image entry points everywhere else. It also owns image ADMISSION, since sniffing means this module reads the file where GTK used to: a local image passes the same regular-file-and-size test a document does (POLICY § Input limits), and the pixel cap is applied before any canvas is allocated. Its four callers are the preview's local and remote images, theme sprites and PDF export. |
| `animation/` | Owns animated-image PLAYBACK, split so that only the GTK-bound pieces need a display. `policy` reconciles the reader's process-wide Play Animations choice with GTK's `gtk-enable-animations` and the platform's own reduce-motion source ("reduce animations" wins), and notifies subscribers when any of the three changes; `schedule` is the pure decision core over the frame clock — which frame is due, skip rather than queue when a decode overruns, and the stop after the final play; `worker` moves a `richimg::Animation` onto GLib's I/O pool, one decode per animation in flight by construction and capped so animations cannot starve the pool document I/O and the snapshot writer share (GTK4Rs/AP-243); `source` shares one copy of a file's encoded bytes by weak reference among every picture showing it; `tick` is the typed frame-clock registration, which REMOVES itself on drop because `gtk::TickCallbackId` does not (a dropped handle leaves an unremovable callback pinning the whole toplevel's clock at display rate); `paintable` is the `GdkPaintable` that swaps a texture per presented frame, with its `drive` half owning the tick and its `badge` half the paused overlay; `visibility` decides whether anyone can see a picture at all, by intersecting allocations in widget space rather than by `is_mapped()`; and `sprites` applies the same machinery to a theme's sprites, which are painted rather than pictured: every widget that draws one — the preview's paint plan, the rule, the table header, the disclosure indicator — owns a table of them and hands its paint a frame source, so a sprite plays wherever a theme puts it. |
| `clipboard.rs` | Owns what the **editor** puts **on** a clipboard: plain text, never GTK's default rich `GtkTextBuffer`. That is what keeps a same-application paste to a SINGLE `insert-text` emission — rich content is re-inserted one chunk per syntax-highlight tag toggle, and a toggle inside a `\r\n` is the mechanism behind ScrAP-312. CLIPBOARD is covered on both panes (this module overrides `copy-clipboard`/`cut-clipboard` on the editor; the preview publishes plain text by its own separate route in `preview/interactions.rs`). PRIMARY is **not** taken over on either pane — GTK republishes it on every selection change, and the publisher-side takeover this module once used could not survive the preview's own re-render-swaps-its-buffer behaviour, so it was replaced with a consumer-side fix: the editor's middle-click paste reads PRIMARY as text rather than as a buffer, which is safe against *any* publisher's content, including the preview's still-un-taken-over rich one. GTK4Rs/AP-318's PRIMARY-selection buffer leak is a GTK-core defect this module cannot reach either way. Also owns `set_text`, **the one writer of clipboard text in the application** (`clippy.toml` bans `gdk::Clipboard::set_text` elsewhere): GDK's own write first, so in-application pastes read the new text, then on macOS `platform::mac::pasteboard`'s eager write, before any main-loop turn. GTK's own built-in Copy/Cut on a text field or a selectable label does not pass through it; for those, `wire_eager_copy_for_field` (called by `widgets/textfield.rs`, which builds every field) and `wire_eager_copy_for_label` connect AFTER GTK's handler and republish the copied text eagerly on macOS. |
...
| `palette/` | Owns the resolved preview colour set and the WCAG colour maths that derives it: one `probe_named` for the desktop's named-colour chain and its floor, one `walk_to_contrast` lifting an ink to the AA floor from either polarity. Split `mod`/`tests` at the file-size limit. Its `codechips` child owns the inline-code chip **per SURFACE** — the page, each banded heading level, a filled quote panel, a table's header cell — because a chip derived from the page is wrong wherever something else is drawn behind the run (ScrAP-356); it also owns `surface_at`, the one definition of which surface a run is on, read by the tag choice and both export sinks so the four renderings cannot answer differently. |
...
| `session/` | Owns cross-session persistence, split four ways: `mod.rs` the read/write entry points and the save freeze, `schema.rs` the persisted structs **and the scope each value is held at** (app-wide / per window / per tab), `migrate.rs` every superseded shape, `statedir.rs` the state-directory lookup — the single one in the tree, shared with the forensic log and crash reports. |
| `session/migrate.rs` | Owns reading files written by superseded schema versions (currently v1–v5). **The one place a new schema version is wired in**: it exposes a replace-the-parse entry for v1 and one additive entry for everything later, so `mod.rs` never learns which versions exist. Each migration is inert on a file the current writer produced, which is why none of them needs a version test — and the module doc records the single condition that inertness depends on. |
...
| `docio/` | Owns every read and write of a user document, and the fact that none of them touch the main thread: the blocking halves are module-private, so its `async fn`s are the crate's only route to a document. `docio/pool.rs` owns the dispatch and one admission gate over GLib's shared I/O pool (GTK4Rs/AP-243); `docio/budget.rs` owns how much of that pool the whole application may hold, because three gates admit to it and the quantity that must stay under the cliff is their sum plus the crash-recovery writer's reserve — which no single gate can check, and which was previously stated only in a doc comment and had gone false. `docio/rename.rs` owns the rename primitive and the filename rules, taking the platform's rules as **data** so both platforms' are unit-testable from either — and documents that GIO's destination refusal is a `lstat`-then-`rename`, i.e. a narrowed race rather than a guarantee. Being the only route is what lets the read door carry one cross-cutting obligation for free: a document absent from disk is checked for the debris a crash mid-rename strands, so Open, session restore, link navigation and crash recovery all recover it without any of them knowing about renames (GEP-67). |
| `app/coldstart.rs` | Owns the one question both `GApplication` entry points must agree on — *is this activation the process's cold start?* — as a claim taken **synchronously**, true for exactly one caller. It is a module rather than a line in either handler because those files are GTK wiring and outside the coverage gate, and because the defect it replaces was a predicate over observable state (*no windows exist yet*) that stayed true for the whole of the interval the work it authorised occupied, so a second launch during session restore restored everything twice. Asking IS claiming: there is no way to read the answer without consuming it. |
...
| `window/find/matcher.rs` | Owns **where a query occurs in a text**, for every text the preview searches — its buffer, its table-cell labels, and the source of its collapsed disclosures, which used to carry three hard-wired matchers between them. Compiles a query under the reader's options to a literal or a `glib::Regex` (GRegex, the engine the editor's own engine compiles with, so a pattern cannot mean two things in two panes) and answers byte ranges. GTK-free. |
| `window/find/options.rs` | Owns the three match options as one value, and the table naming the `win.` action that carries each — so the find-bar toggle, the Edit-menu item and the session field cannot disagree about which action name means which boolean. |
| `window/find/history.rs` | Owns a find-bar field's **committed-entry history** — bounded, de-duplicated, most recent first — for both fields, plus the row label (shortened and single-lined, because an entry is a string the reader typed). Persisted with the tab, and repaired rather than rejected on load. GTK-free. |
| `window/find/scope.rs` | Owns the shape of the **search-in-selection** bound and the arithmetic over it — whether a match fits inside the passage, which entry stepping lands on and how it wraps *within* the passage, and where the reader currently is in it. GTK-free; the two panes' live bounds (a pair of `GtkTextMark`s, a char range plus its render key) are assembled in `window/find.rs`. |
| `window/find/bodytext.rs` | Owns making the preview's buffer searchable by the matcher, and mapping a match back to buffer offsets. Two traps live here and nowhere else: `text()` omits anchored children so its offsets are not the buffer's (ScrAP-74), and the `U+FFFC` placeholders `slice()` yields instead must not be matchable. GTK-free. |
...
| `saferizer/` | Owns the project-local typed seams that promote GTK's prose-only runtime contracts to compile-time ones — including `popover_anchor`, the single gate every popover anchor passes through (on-viewport proof plus its clamp, inseparably) and the sanctioned reader for GTK's `pointing_to` out-parameter; and `click_activation`, which owns *both* halves of every self-drawn affordance's click — its pairing rule lives in a display-free state machine, leaving the GTK closures decision-free (GTK4Rs/AP-169); and `scrollpos`, the one route by which a scroll position is written, because a plain adjustment write is an unconditional supersede that cancels any scroll animation in flight and GTK exposes no way to ask whether one is (GTK4Rs/AP-260). And `file_monitor`, whose `DocMonitor` owns a document's live-reload monitor and is deliberately not `Clone`, so cancelling one consumes it: releasing a cancelled `GFileMonitor` after a main-loop dispatch aborts the process on Windows, and nothing but the type can keep a second reference from being parked across a turn (GTK4Rs/AP-340). |
| `workaround.rs` | Owns the per-process `XDG_CONFIG_HOME` redirect that defends against the GTK 4.6 XCompose crash (GTK4Rs/AP-3). |
| `platform/` | Owns the cross-platform seams whose answer differs per OS, behind one façade each so no caller carries a platform match. `system_reduced_motion` is the current one: it answers `Some(true)`/`Some(false)` where the host has a real preference and `None` — "no opinion", resolved to *not reduced* — where it has none (TDD 27.10). Windows reads it from Win32; Linux abstains because `GtkSettings` already carries the desktop's answer; macOS abstains pending `sdd/PLAN.accessibility.md`. |
| `platform/win32/` | Owns the project's **entire** Win32 surface (`#[cfg(windows)]`, gated once at its declaration in `platform/mod.rs`), split by cause and not by DLL: `frame` (the dark DWM title bar, and keeping GTK's remembered window size current while the OS holds the window maximized), `appearance` (reading the Windows light/dark setting GTK does not read itself), `privacy` (an owner-only state directory, which `std::fs` cannot express here), `process` (whether a pid is a running instance — Windows has no `/proc`), `reduced_motion` (the OS-level "show animations" preference, `SPI_GETCLIENTAREAANIMATION`, which GTK does not surface at this project's 4.6 floor — the `gtk-interface-reduced-motion` setting that would is 4.22). Every name is re-exported from its `mod.rs`, so callers address it as one module. It detects and hands off; it themes nothing directly. |
...
(GEP-15), so what it records at the moment of death is the whole of the evidence.
...
would unhandled (GEP-35); and every artefact is created owner-only through one
