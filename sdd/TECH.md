...
| `preview/`, `codeview/`, `decorplan.rs`, `affordance.rs`, `keynav.rs`, `farscroll.rs` | The read-only preview: its construction, interaction, scroll and CSS wiring; `CodePreviewView`, the `GtkTextView` subclass that self-draws every decoration (one painter per decoration, plus the annotation card popover); the display-free paint plan, affordance geometry and key-navigation decisions those painters read; and every far scroll, re-issued only against a settled layout. |
