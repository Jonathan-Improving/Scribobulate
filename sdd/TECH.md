...
| `palette/` | Owns the resolved preview colour set and the WCAG colour maths that derives it: one `probe_named` for the desktop's named-colour chain and its floor, one `walk_to_contrast` lifting an ink to the AA floor from either polarity. Split `mod`/`tests` at the file-size limit. |
