...
| `session.rs` | Owns cross-session persistence: the session schema (**v6**, one reader per superseded shape so each migration is inert on a file the current writer produced), reading and writing the state file, the **scope** each persisted value is held at (app-wide, per window, per tab), and — as the single lookup in the tree — the user **state-directory** resolution the forensic log and crash reports share with it. |
